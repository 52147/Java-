...
## Session 與 Cookie
Http 是一種無狀態的協議，無狀態協議代表瀏覽器不會保存任何會話信息，所以無法確保每一次的會話是同一個用戶發出，為了確認訪問者信息，瀏覽器會和服務端進行"會話跟蹤"，代表在進行一些特殊用戶權限才有的操作時，會將用戶狀態用cookie 或session保存起來，所以cookie和session 是用來儲存用戶狀態的機制。    
