...
1. npm install
   - 安裝所有的 dependencies (在package.json中)在 node_modules 目錄下
...
4. npm install moduleName -g
   ...
   - 不會將模塊依賴寫入devDependencies 或 dependencies 節點
   - 運行npm install 初始化項目時，不會下載模塊
5. npm install moduleName --save
   - 運行npm install 初始化項目時，安裝模塊到node_modules 目錄下
   - 會將模塊寫入dependencies 節點
   - 運行npm install 初始化項目時，會安裝模塊到項目目錄下
   - 運行npm install--production 時，會自動下載dependencies模塊到node_modules 目錄下
6. npm install moduleName --save-dev
   - 運行npm install 初始化項目時，安裝模塊到node_modules 目錄下
   - 不會將模塊依賴寫入devDependencies 節點
   - 運行npm install--production 時，不會自動下載devDependencies模塊到node_modules 目錄下     
